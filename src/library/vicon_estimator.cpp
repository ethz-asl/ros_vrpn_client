...
ViconEstimator::ViconEstimator() : translational_estimator_(), rotational_estimator_() {}
...
RotationalEstimator::RotationalEstimator() : estimator_parameters_(), estimator_results_()
{
  // Resetting the estimator
  reset();
}
...
void RotationalEstimator::reset()
{
  ...
  covariance_ << estimator_parameters_.dorientation_estimate_initial_covariance_ *
  ...
  measurement_covariance_ << estimator_parameters_.orientation_measurement_covariance_ *
...
Eigen::Quaterniond RotationalEstimator::getEstimatedOrientation() const
{
  ...
  if (estimator_parameters_.output_minimal_quaternions_) {
    ...
    orientation_for_return =
      Eigen::Quaterniond(correction_factor * orientation_estimate_B_W_.coeffs());
  } else {
...
EstimatorStatus RotationalEstimator::updateEstimate(
  const Eigen::Quaterniond & orientation_measured_B_W, const double timestamp)
{
  ...
  if (
    estimator_parameters_.outlier_rejection_method_ ==
    OutlierRejectionMethod::MAHALANOBIS_DISTANCE) {
    measurement_update_flag = !detectMeasurementOutlierMahalanobis(orientation_measured_B_W, P_p);
  } else if (
    estimator_parameters_.outlier_rejection_method_ ==
    OutlierRejectionMethod::SUBSEQUENT_MEASUREMENTS) {
    measurement_update_flag = !detectMeasurementOutlierSubsequent(orientation_measured_B_W);
  } else {
...
void RotationalEstimator::updateEstimatePropagateGlobalEstimate(
  const Eigen::Matrix<double, 7, 1> & x_old, const double dt, Eigen::Matrix<double, 7, 1> * x_p)
{
  // Argument checks
  rcpputils::require_true(x_p != nullptr, "x_p is nullptr");
  ...
  Eigen::Quaterniond rate_estimate_quaternion =
    Eigen::Quaterniond(0, rate_estimate_old.x(), rate_estimate_old.y(), rate_estimate_old.z());
  ...
  Eigen::Quaterniond orientation_estimate_priori = Eigen::Quaterniond(
    ...
    (0.5 * (orienation_estimate_old * rate_estimate_quaternion).coeffs()) * dt);
...
void RotationalEstimator::updateEstimatePropagateErrorEstimate(
  const Eigen::Matrix<double, 6, 1> & dx_old, const Eigen::Matrix<double, 7, 1> & x_old,
  const double dt, Eigen::Matrix<double, 6, 1> * dx_p)
{
  // Argument checks
  rcpputils::require_true(dx_p != nullptr, "dx_p is nullptr");
  ...
  Eigen::Vector3d drate_estimate_priori = drate_estimate + drate_estimate_priori_roc * dt;
...
void RotationalEstimator::updateEstimatePropagateErrorCovariance(
  Eigen::Matrix<double, 6, 6> & covariance_old, const Eigen::Matrix<double, 7, 1> & x_old,
  const double dt, Eigen::Matrix<double, 6, 6> * covariance_priori)
{
  // Argument checks
  rcpputils::require_true(covariance_priori != nullptr, "covariance_priori is nullptr");
  // Extracting components of the state
  ...
  A << -1 * skewMatrix(rate_estimate), 0.5 * Eigen::Matrix3d::Identity(), Eigen::Matrix3d::Zero(),
    Eigen::Matrix3d::Zero();
  L << Eigen::Matrix3d::Identity(), Eigen::Matrix3d::Zero(), Eigen::Matrix3d::Zero(),
    Eigen::Matrix3d::Identity();
  ...
  Eigen::Matrix<double, 6, 6> A_d = Eigen::Matrix<double, 6, 6>::Identity() + A * dt;
  ...
  *covariance_priori = covariance_priori_propagation + covariance_priori_addition;
...
void RotationalEstimator::updateEstimateUpdateErrorEstimate(
  const Eigen::Quaterniond & orientation_measured, const Eigen::Matrix<double, 7, 1> & x_priori,
  const Eigen::Matrix<double, 6, 1> & dx_priori,
  const Eigen::Matrix<double, 6, 6> & covariance_priori, Eigen::Matrix<double, 6, 1> * dx_m,
  Eigen::Matrix<double, 6, 6> * covariance_measurement)
{
  // Argument checks
  rcpputils::require_true(dx_m != nullptr, "dx_m is nullptr");
  rcpputils::require_true(covariance_measurement != nullptr, "covariance_measurement is nullptr");
  ...
  Hdq << orientation_estimate_priori.w() * Eigen::Matrix<double, 3, 3>::Identity() +
  ...
  if (error_orientation.w() >= 0) {
    ...
    orientation_predicted =
      Eigen::Quaterniond(Hdq * dorientation_estimate_priori + orientation_estimate_priori.coeffs());
  } else {
...
void RotationalEstimator::updateEstimateRecombineErrorGlobal(
  const Eigen::Matrix<double, 7, 1> x_priori, Eigen::Matrix<double, 7, 1> * x_measurement,
  Eigen::Matrix<double, 6, 1> * dx_measurement)
{
  // Argument checks
  rcpputils::require_true(x_measurement != nullptr, "x_measurement is nullptr");
  rcpputils::require_true(dx_measurement != nullptr, "dx_measurement is nullptr");
  ...
  Eigen::Quaterniond orientation_estimate_measurement =
    orientation_estimate_priori * dorientation_estimate_measurement_quaternion;
...
bool RotationalEstimator::detectMeasurementOutlierMahalanobis(
  const Eigen::Quaterniond & orientation_measured, const Eigen::Matrix<double, 6, 6> & covariance)
{
  ...
  // direct inverse).
  Eigen::Quaterniond q_Z_B = orientation_measured * orientation_estimate_B_W_.inverse();
  ...
  measurement_outlier_flag =
    q_Z_B_mahalanobis_distance >= estimator_parameters_.outlier_rejection_mahalanobis_threshold_;
  ...
  const Eigen::Quaterniond & orientation_measured)
{
  ...
  // measurements
  Eigen::Quaterniond q_Z_Z1 = orientation_measured * orientation_measured_old_.inverse();
  // Calculating the quaternion magnitude
  double q_Z_Z1_magnitude = quaternionRotationMagnitude(q_Z_Z1);
  // Detecting outlier
  measurement_outlier_flag =
    q_Z_Z1_magnitude >=
    estimator_parameters_.outlier_rejection_subsequent_threshold_degrees_ * M_PI / 180.0;
  ...
  if (outlier_counter_ >= estimator_parameters_.outlier_rejection_subsequent_maximum_count_) {
...
double RotationalEstimator::quaternionRotationMagnitude(const Eigen::Quaterniond & rotation)
{
...
void RotationalEstimator::makeCovarianceSymmetric(Eigen::Matrix<double, 6, 6> * covariance)
{
  // Argument checks
  rcpputils::require_true(covariance != nullptr, "covariance is nullptr");
