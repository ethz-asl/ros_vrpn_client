...
public:
  ...
  EstimatorStatus updateEstimate(const Eigen::Vector3d & pos_measured, const double timestamp);
  // Reset the estimator
  void reset();
...
private:
  ...
  void updateEstimatePropagateErrorEstimate(
    ...
    const double dt, Eigen::Matrix<double, 6, 1> * dx_priori);

  void updateEstimatePropagateErrorCovariance(
    Eigen::Matrix<double, 6, 6> & cov_old, const Eigen::Matrix<double, 7, 1> & x_old,
  ...
  bool detectMeasurementOutlierSubsequent(const Eigen::Quaterniond & orientation_measured);
