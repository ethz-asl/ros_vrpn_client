...
void generateRotationalTrajectorySinusoidal(double quatTrajectory[][4], double omegaTrajectory[][3], const int trajectoryLength)
{
	...
	for(int i=0; i<trajectoryLength-1; i++)
	{
		...
		Eigen::Quaterniond diff = Eigen::Quaterniond( ( q_k.coeffs() - q_k_1.coeffs() ) / ROT_TRAJECTORY_DT ) ;
...
TEST(rotationalEstimator, sinusoidal_clean)
{
	...
	rotationalEstimatorParameters.dQuat_hat_initialCovariance = 1;
	rotationalEstimatorParameters.dOmega_hat_initialCovariance = 1;
	rotationalEstimatorParameters.dQuat_processCovariance = 0.01;
	rotationalEstimatorParameters.dOmega_processCovariance = 1000*1;
	rotationalEstimatorParameters.quat_measurementCovariance = 0.0005;
  ...
  EXPECT_NEAR(quatError[2], 0, QUAT_ERROR_THRESHOLD) << "Z errorquaternion estimate error too great" ;
	...
	// Calculating velocity estimate errors
	double omegaError[3] ;
	calculate3dRmsError(omegaError, omegaTrajectory, omegaEstTrajectory, trajectoryLength, startIndex);
